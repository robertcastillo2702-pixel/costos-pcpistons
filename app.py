    ...
    if API_KEY == "TU_CLAVE_API_AQUI":
        ...
        try:
            ...
            if not prod_ids:
                ...
                # =========================================================
                # CORRECCIÓN INTELIGENTE DE LA UNIDAD DE MEDIDA
                uom_raw = prod_data.get('uom_id', [0, ''])[1].strip()
                
                # 1. Cambiar "Pieza X" por "Caja de "
                if "Pieza X" in uom_raw:
                    uom_raw = uom_raw.replace("Pieza X", "Caja de")
                
                # 2. Corregir el error de Odoo donde el 12 realmente es un 8
                if "12" in uom_raw:
                    uom_raw = uom_raw.replace("12", "8")
                
                # 3. Asegurar que no falte la palabra "de" (ej. "Caja 8" -> "Caja de 8")
                uom_raw = re.sub(r'(?i)Caja\s+(\d+)', r'Caja de \1', uom_raw)
